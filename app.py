...
# كود CSS لضبط المحاذاة الداخلية لليمين
st.markdown(
    """
    <style>
    /* محاذاة كل النصوص لليمين */
    .stMarkdown, .stText, p, div, label { text-align: right !important; direction: rtl !important; }
    ...
    with col_data:
        ...
        if calc_button:
            with st.spinner('جاري الحساب...'):
                ...
                st.markdown(
                    f"<div dir='rtl' style='text-align: right; color: green; font-weight: bold; margin-bottom: 10px;'>"
                    f"تم الحساب بنجاح! إجمالي عدد الحضور: {int(total_attendance)} طالب"
                    f"</div>", 
                    unsafe_allow_html=True
                )
